...
def submit(chuncks):
	...
	for job_no, chunck in enumerate(chuncks, start=1):
		...
		cmd = "python {} --file_job {} --path_epi_results {}".format(script2call, file_job, path_epi_results) # OBS: path_epi_results is NOT variable
...
#n_jobs_per_bsub = 60 # --> WEEK
n_jobs_per_bsub = 25 # --> HOUR
...
if not os.path.exists(file_json_config):
	...
	logger.info( "CONFIG FILE EXISTS. Will *NOT* overwrite it." )

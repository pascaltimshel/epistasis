############### SYNOPSIS ###################
# class: function
# description: this script will generate a df/matrix containing indexes for the null distribution
# remarks: none.
...
perm_restricted <- function(n_perm, df.interaction_table) {
  ...
  #################################### Algorithm ####################################
  #   init df.perm [interactions X n_perm];
  #   [check that there are no duplicates in the data set]
  #   for each partnerB_hic in interaction table:
  #     f1) select partnerB that does not form interaction pairs in the interaction table. [needed to avoid false positives].
  #     f2) select partnerB *NOT* on same chromosome as partnerA
  #     f3) select partnerB meeting distance criteria. [needed to avoid false positives]
  #     f4) count pool_valid_partners.
  #     f5) warn if n_perms > pool_valid_partners. If this inequality is true, the null will contain “duplicate null interaction pairs”.
  #     f6) sample without replacement n_perms from the pool. 
  #     f7) Save indecies as row in df.perm.
  #   return df.perm
  #   
  ### Suggested downstream validation/reports
  #   validation: check that df.perm {partnerB_hic != partnerB_null}
  #   report: number of “duplicate null interaction pairs"
